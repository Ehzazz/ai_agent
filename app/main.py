...
def retrieve_docs(state: AgentState) -> AgentState:
    user_id = state.get("user_id")  # Add user_id to AgentState and pass it in
    retriever = vectorstore.as_retriever(
        search_kwargs={
            "k": 5,
            "filter": {"user_id": user_id}  # Only fetch docs for this user
        }
    )
...
def embed_docx(file_obj, metadata: dict = {}):
    """Loads and embeds a DOCX file, storing its vectors in PostgreSQL via pgvector."""
    import tempfile
    from langchain_text_splitters import RecursiveCharacterTextSplitter
    from langchain_community.vectorstores.pgvector import PGVector as BasePGVector
    class NoCreatePGVector(BasePGVector):
